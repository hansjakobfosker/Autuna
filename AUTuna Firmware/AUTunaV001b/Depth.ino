// this code is mostly grabbed from OpenROV. www.openrov.com.
/*
...
float Temperature, Pressure, atmosphericPressure = 0, TempDifference, Offset, Sensitivity;
...
void checkDepth(){
  ...
  depth = pressureReading();
 /// Serial.print("Pressure in Bars is: ");
 // Serial.print(depth);
  //Serial.print(", depth in meters is: ");
  depth = depth * 10;
  //Serial.println(depth); 
  //Serial.println();
  
  if(depth > targetDepth && pecFinAngle < 120){
    
    pecFinAngle += 1;
  
  }
  
    if(depth < targetDepth && pecFinAngle > 60){
 ...
 */ 
  ...
  */
  ...
  return Pressure - atmosphericPressure;
  }
  
  // calibrate for atmospheric pressure
  void measureAtmosphere(){
  
  atmosphericPressure = pressureReading();
  }
